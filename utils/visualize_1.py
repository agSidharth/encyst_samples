...
class Visualizer():
    ...
    def encystSamples(self,classifier,samples_per_dim,from_natural,rate,max_iterations=100):
        ...
        print('ALERT: BRUTE FORCE----------------------------------x-------------------------------')
        PATH = 'classifers/net_architecture.pth'
        attacked_classifier = torch.load(PATH,map_location=torch.device('cpu'))

        PATH = 'classifers/square_white_tar0_alpha0.00_mark(3,3).pth'
        attacked_classifier.load_state_dict(torch.load(PATH,map_location=torch.device('cpu')))
        ...
        for dim in range(self.latent_dim):
            ...
            for sample_num, sample in enumerate(samples):
                ...
                _,dirty_pred = torch.max(attacked_classifier((img).to(self.device)),1)
